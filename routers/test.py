...
async def get_join(db: Session = Depends(get_db)):
    """post join of workers and orders"""
    data = await TestService.get_join_of_workers_and_orders(db)
    return data


@router.post("/test/count", tags=["Test"])
...
@router.post("/test/generate_workers/{quantity}", tags=["Generators"])
...
@router.post("/test/generate_projects/{quantity}", tags=["Generators"])
...
@router.post("/test/generate_orders/{quantity}", tags=["Generators"])
